...
export const rejectOrder = async (id, note, user, order) => {
  ...
  const email = await axios.post(`http://localhost:9123/v1/send`, {
    ...
    mail: {
      ...
      outro:
        'Please consider leaving your feedback so we can improve our service.', //ending text
    },
  });
  return res !== null;
};
...
export const grantAdmin = async (user, grant) => {
  ...
  const email = await axios.post(`http://localhost:9123/v1/send`, {
    to: user.email, //reciever email address
    subject: `${
      grant ? 'Admin Privileges Granted!' : 'Admin Privileges Revoked!'
    }`, //email subject
    mail: {
      //this will be used to generate the email body
      header: user.name, //Bold header of email body
...
export const getDailySales = async (year, month, setter) => {
  await axios
    .get(`http://localhost:9122/v1/sales/daily/${year}/${month}`)
    .then((res) => {
      try {
        setter((prev) => ({
          ...prev,
...
export const getMonthlySales = async (year, setter) => {
  await axios
    .get(`http://localhost:9122/v1/sales/monthly/${year}`)
    .then((res) => {
      try {
        setter((prev) => ({
          ...prev,
...
export const getYearlySales = async (setter) => {
  await axios.get(`http://localhost:9122/v1/sales/yearly`).then((res) => {
    try {
      setter((prev) => ({
        ...prev,
        sales: {
...
export const calculateSales = async (filter, dateItems) => {
  ...
  switch (filter) {
    case 'daily':
      res = await axios.post(`http://localhost:9122/v1/sales/daily`, {
        year: dateItems.year,
        month: dateItems.month,
      });
      break;
    case 'monthly':
      res = await axios.post(`http://localhost:9122/v1/sales/monthly`, {
        year: dateItems.year,
      });
      break;
    case 'yearly':
      ...
      break;
    case 'orders':
      res = await axios.post(`http://localhost:9122/v1/orders/monthly`, {
        year: dateItems.year,
        month: dateItems.month,
      });
    default:
...
export const getMonthlyOrders = async (year, month, setter) => {
  await axios
    .get(`http://localhost:9122/v1/orders/${year}/${month}`)
    .then((res) => {
      try {
        setter((prev) => ({
          ...prev,
