...
export default function ShopOrders() {
  ...
  const getOrdersbySeller = async () => {
    try{
      const response = await axios(`http://localhost:9124/api/orders/seller/products/${userId}`);
  ...
  const handleShippedOrder = async (e, orderId) => {
    e.preventDefault();
    try{
      const response = await axios.put(`http://localhost:9124/api/orders/${orderId}/shipped`, { sellerId: userId });
      const order =  response.data;
      setOrders(order);
      alert("Order Shipped");
    ...
    <div className="p-1 shadow-md text-white">
      <div className="overflow-x-auto" 
        ...
        <table className="w-full border-collapse text-center text-grey-400">
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order._id} className="transition-all bg-slate-800 border-b-2 border-slate-600 m-10 hover:bg-slate-700">
                <td className="px-6 py-4 whitespace-nowrap">{order.createdAt.substring(0, 10)}</td>
                <td className="px-6 py-4 whitespace-nowrap">{order._id}</td>
                <td className="px-6 py-4 whitespace-nowrap text-center">{order.orderItems.map((product) => (
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {(order.isShipped) ? <div className="text-primarylight">Shipped</div> : null}
                </td>
                ...
                <td className="px-6 py-4 flex">
                  {
                    ...
                    : <div className="italic text-slate-500">
                        {(order.isShipped) && `Shipped on ${order.shippedAt.split("T")[0]}`}
                    </div>
                  }
              </td>
              </tr>
